...
void setNonBlocking(int fd) {
    int flags = fcntl(fd, F_GETFL, 0);      // 先读出这个fd当前的flag设置
    fcntl(fd, F_SETFL, flags | O_NONBLOCK); // 在原有flag基础上，加上"非阻塞"这一位
}
...
int main() {
    ...
    // events数组：epoll_wait每次返回时，会把"就绪的fd列表"填到这个数组里
    ...
    while (true) {
        ...
        //   参数-1表示一直等下去，不设超时；返回值n是"这次有多少个fd同时就绪了"
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (current_fd == listen_fd) {
                // 为什么要用while循环accept，而不是accept一次就完事？
                // 因为epoll通知的是"listen_fd上有事件"，但这一瞬间可能同时有好几个客户端
                // 在排队等待连接（尤其是高并发场景），如果只accept一次，
                // 剩下排队的连接要等到下一轮epoll_wait才会被处理，效率较低。
                // 所以规矩是：收到listen_fd就绪通知后，要把当前所有能接的连接一次性接完，
                // 直到accept返回-1（代表"暂时没有更多新连接了"）为止。
                while (true) {
                    ...
                    client_ev.events = EPOLLIN;   // 关心"客户端发数据过来了"这个事件
